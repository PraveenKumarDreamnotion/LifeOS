...
const INLINE_RULES = [
  { kind: "code", re: /`([^`]+)`/ },
  // code first: its contents are literal (no nested formatting)
  { kind: "bold", re: /\*\*([\s\S]+?)\*\*/ },
  { kind: "link", re: /\[([^\]]+)\]\(([^)]*)\)/ },
  { kind: "italic", re: /\*([^*\n]+?)\*/ }
];
function renderInline(text, keyPrefix) {
  ...
  while (rest.length > 0) {
    ...
    if (best.kind === "code") {
      out.push(
        /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "md-code", children: best.groups[1] }, key)
      );
    } else if (best.kind === "bold") {
      out.push(/* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: renderInline(best.groups[1], key) }, key));
    } else if (best.kind === "italic") {
      out.push(/* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: renderInline(best.groups[1], key) }, key));
    } else {
      out.push(
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "md-link", children: renderInline(best.groups[1], key) }, key)
      );
    }
...
function BlockView({ block }) {
  if (block.type === "heading") {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { className: `md-h md-h${level}`, children: renderInline(block.text, "h") });
  }
  if (block.type === "ul") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "md-ul", children: block.items.map((it, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: renderInline(it, `uli${i}`) }, i)) });
  }
  if (block.type === "ol") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { className: "md-ol", start: block.start, children: block.items.map((it, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: renderInline(it, `oli${i}`) }, i)) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "md-p", children: block.lines.map((line, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(reactExports.Fragment, { children: [
    i > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}),
    renderInline(line, `p${i}`)
  ] }, i)) });
}
function Markdown({ text, className }) {
  const blocks = reactExports.useMemo(() => parseBlocks(text), [text]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: className ? `md ${className}` : "md", children: blocks.map((b, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(BlockView, { block: b }, i)) });
}
const SILENCE_STOP_MS = 2e3;
const HARD_CAP_MS = 3e4;
const SPEECH_RMS_THRESHOLD = 0.01;
function useSpeech(onFinalTranscript, bridge = window.lifeos.speech, beforeStart) {
  const [state, setState] = reactExports.useState("idle");
  const [partial, setPartial] = reactExports.useState("");
  const [errorMsg, setErrorMsg] = reactExports.useState(null);
  const [supportsPartials, setSupportsPartials] = reactExports.useState(true);
  const [volume, setVolume] = reactExports.useState(0);
  const streamRef = reactExports.useRef(null);
  const ctxRef = reactExports.useRef(null);
  const nodeRef = reactExports.useRef(null);
  const capTimer = reactExports.useRef(null);
  const silenceTimer = reactExports.useRef(null);
  const activeRef = reactExports.useRef(false);
  const onFinalTranscriptRef = reactExports.useRef(onFinalTranscript);
  const beforeStartRef = reactExports.useRef(beforeStart);
  const bridgeRef = reactExports.useRef(bridge);
  const wantsStopRef = reactExports.useRef(false);
  ...
  const teardown = reactExports.useCallback(async () => {
  ...
  const bumpSilence = reactExports.useCallback(() => {
    if (silenceTimer.current) window.clearTimeout(silenceTimer.current);
  ...
  const start = reactExports.useCallback(async () => {
    ...
    try {
      ...
      const name = e.name;
      setErrorMsg(
        name === "NotAllowedError" ? "Microphone permission denied. You can still type." : name === "NotFoundError" ? "No microphone found. You can still type." : "Microphone unavailable. You can still type."
      );
      setState("error");
      await teardown();
    }
  ...
  reactExports.useEffect(() => () => void teardown(), [teardown]);
